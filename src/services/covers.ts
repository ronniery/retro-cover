...
const getGameCoverMetadataBy = async (gameId: string, options?: GetGameCoverOptions): Promise<GameCoverMetadata> => {
  ...
  if (profile.includes(HOME_TITLE))
    throw new Error("Invalid gameId given")

  return new GameCoverMetadataParser(profile)
    .parse(Object.assign({}, options, { gameId }))
}

const getAllGameCovers = async (drafts: GameCoverMetadata['drafts']) => {
  const coverPromises = drafts.map(async ({ coverId }) => {
    const { data } = await httpApi.get<GameCover>(GAME_COVERS, {
      ...
      transformResponse: (html: string) =>
        new GameCoverParser(html, false).parse()
    })
...
export const getGameCovers = async (gameIds: string | string[], options?: GetGameCoverOptions): Promise<GameCoverCollection> => {
...
export const downloadCovers = async (targets: Array<string | number>, outputPath: string, parallelDownload = 3) => {
  const maxParallelDownloads = Math.min(5, parallelDownload);
  const fullPath = createFullOutputPath(outputPath);

  for (let chunkBlock of chunk(targets, maxParallelDownloads)) {
